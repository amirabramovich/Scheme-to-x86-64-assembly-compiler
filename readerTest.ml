...
let test_ThreeDots_ = fun () ->
    ...
    test 4 (sexpr_eq (execute_read_sexpr "(let ([x 1] [y 2]) [lambda (z) (+ x y z) ...")
      (execute_expected(Pair(Symbol("let"),Pair(Pair(Pair(Symbol("x"),Pair(Number(Int(1)),Nil)),Pair(Pair(Symbol("y"),Pair(Number(Int(2)),Nil)),Nil)),
        Pair(Pair(Symbol("lambda"),Pair(Pair(Symbol("z"),Nil),Pair(Pair(Symbol("+"),Pair(Symbol("x"),Pair(Symbol("y"),
          Pair(Symbol("z"),Nil)))),Nil))),Nil))))));
    test 5 (sexpr_eq (execute_read_sexpr "(#t \"...\" . #F)") 
      (execute_expected(Pair(Bool(true),Pair(String("..."),Bool(false))))));
    (* Failed, fixed *)
    test 6 (sexpr_eq (execute_read_sexpr "(#t \"...\" . #F) ...")
      (execute_expected(Pair(Bool(true),Pair(String("..."),Bool(false))))));
    (* Failed, fixed *)
    test 7 (sexpr_eq (execute_read_sexpr "(#t \"...\" . #F) ...") 
      (execute_expected(Pair(Bool(true),Pair(String("..."),Bool(false))))));
    (* Failed, fixed *)
    test 8 (sexpr_eq_as_list (execute_read_sexprs_as_list "  ... (\"...\") ... [1]") 
      (execute_expected_as_list([Pair(String("..."),Nil); Pair(Number(Int(1)),Nil)])));
    (* Failed, fixed *)
    test 9 (sexpr_eq_as_list (execute_read_sexprs_as_list "  ... (\"...\") ... [1] ...") 
      (execute_expected_as_list([Pair(String("..."),Nil); Pair(Number(Int(1)),Nil)])));
    (* Failed, fixed *)
    test 10 (sexpr_eq_as_list (execute_read_sexprs_as_list " ...  (\"...\") ... () ... [1] ...") 
      (execute_expected_as_list([Pair(String("..."),Nil); Nil; Pair(Number(Int(1)),Nil)])));
    (* Failed, fixed *)
    test 11 (sexpr_eq_as_list (execute_read_sexprs_as_list " ...  (\"...\") ... (;...\n) ... [1] ...") 
      (execute_expected_as_list([Pair(String("..."),Nil); Nil; Pair(Number(Int(1)),Nil)])));
    (* Failed, fixed *)
    test 12 (sexpr_eq_as_list (execute_read_sexprs_as_list " ...  (\"...\") ...  [#\\c . ;...\n #\\d] ... [1] ...") 
      (execute_expected_as_list([Pair(String("..."),Nil); Pair(Char('c'),Char('d')); Pair(Number(Int(1)),Nil)])));
    (* Failed, fixed *)
    test 13 (sexpr_eq_as_list (execute_read_sexprs_as_list " ...  (\"...\") ...  [([#\\c . ;...\n #\\d ... [1] ...") 
      (execute_expected_as_list([Pair(String("..."),Nil); Pair(Pair(Pair(Char('c'),Char('d')),Nil),Nil); Pair(Number(Int(1)),Nil)])));
    ;;
    (* Failed *)
